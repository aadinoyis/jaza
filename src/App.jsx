...
const url = "https://florahomes-backend.herokuapp.com/api/properties/64b3e0bb150457780302efdb"
...
export default function App() {
  const [interestForm,
    setInterestForm] = useState(false)
  const [faqIndex,
    setFaqIndex] = useState(0)
  const [activeFaq,
    setActiveFaq] = useState(false)
  const [startTime] = useState(new Date());
  const [timeLeft,
    setTimeLeft] = useState( {
      ...
      <section className="bg-[#105C04] pt-24 pb-4 px-4 lg:px-48">
        <div className="auto-container">
          ...
          <div className="border-[#ffc20b] border-2 shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px] py-2 px-2">
            ...
            ?
            <img src={introduction.media.src} alt="jaza homes" width={"100%"} height={"auto"} />:
          <video width="100%" autoPlay controls>
            <source src={introduction.media.src} type="video/mp4" />
        </video>
        }
      </div>
      <br /><br />
...
<img src={features.media} alt="jaza homes" width={"100%"} height={"auto"} />
...
<p>
Own one apartment fully for yourself OR co-own in slot(s) to earn annual rental or one-time 12 months resell income.
</p>
...
<h1 className="text-[1.5rem] lg:text-[2.5rem] font-medium ">{apartment.title}</h1>
<p className="border-solid border-t-4 border-[#ffc20b] max-w-[600px]">
...
<img src={apartment.media} alt="jaza homes" width={"100%"} height={"auto"} />
</div>
